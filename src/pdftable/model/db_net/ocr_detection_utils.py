...
def boxes_from_bitmap(pred, _bitmap, dest_width, dest_height):
    ...
    assert _bitmap.size(0) == 1
    bitmap = _bitmap.cpu().numpy()[0] if not isinstance(_bitmap, np.ndarray) else _bitmap[0]
    pred = pred.cpu().detach().numpy()[0] if not isinstance(pred, np.ndarray) else pred[0]
    height, width = bitmap.shape
    boxes = []
    scores = []

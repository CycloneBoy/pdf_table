...
# -*- coding: UTF-8 -*-
# @Project ：PdfTable 
# @File    ：table_extractor_lattice
# @Author  ：cycloneboy
# @Date    ：20xx/5/29 16:12
import copy
import os
import warnings
...
class TableExtractorLattice(TableExtractorBase):
    ...
    def __init__(
        ...
        super().__init__(debug=kwargs.get('debug', True))
    ...
    def extract_tables(self, filename, layout_kwargs=None, **kwargs):
        ...
        if not self.horizontal_text:
            if self.images:
                warnings.warn(f"{self.basename} is image-based, camelot only works on text-based pages.")
            else:
                warnings.warn("No tables found on {}".format(self.basename))
            return []
    ...
    def _generate_table(self, table_idx, cols, rows, **kwargs):
        ...
        for direction in ["vertical", "horizontal"]:
            for t in self.t_bbox[direction]:
                ...
                if indices[:2] != (-1, -1):
                    ...
                    indices = TableExtractorLattice._reduce_index(
        ...
        if self.copy_text is not None:
            table = TableExtractorLattice._copy_spanning_text(table, copy_text=self.copy_text)

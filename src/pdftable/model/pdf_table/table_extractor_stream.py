...
# -*- coding: UTF-8 -*-
# @Project ：PdfTable 
# @File    ：table_extractor_stream
# @Author  ：cycloneboy
# @Date    ：20xx/5/31 13:33
import os
import warnings
...
from .table_core import TextEdges, Table
...
__all__ = [
    'TableExtractorStream'
]


class TableExtractorStream(TableExtractorBase):
    ...
    def __init__(self, table_regions=None, table_areas=None, columns=None, split_text=False, flag_size=False,
                 strip_text="", edge_tol=50, row_tol=2, column_tol=0, **kwargs):
        super().__init__()
    ...
    def _add_columns(cols, text, row_tol):
        ...
        """
        if text:
            text = TableExtractorStream._group_rows(text, row_tol=row_tol)
            elements = [len(r) for r in text]
            new_cols = [
                (t.x0, t.x1) for r in text if len(r) == max(elements) for t in r
            ]
            cols.extend(TableExtractorStream._merge_columns(sorted(new_cols)))
        return cols
        ...
        if self.table_areas is None:
            ...
            if self.table_regions is not None:
                ...
                for region in self.table_regions:
                    ...
                    x2 = float(x2)
                    y2 = float(y2)
                    region_text = PdfUtils.text_in_bbox((x1, y2, x2, y1), self.horizontal_text)

...
# -*- coding: utf-8 -*-
# @Project ：PdfTable 
# @File     : gpma_mask_head.py
# @Author   : cycloneboy
# @Date     : 20xx/9/23 - 22:01
import torch
import torch.nn as nn
import torch.nn.functional as F

from .base_utils import imresize
from .mmcv_utils import ConvModule
    ...
    def get_seg_masks(self, preds, img_metas, pad_shape):
        ...
        for i, meta in enumerate(img_metas):
            ...
            if isinstance(mask_pred, torch.Tensor):
                mask_pred = mask_pred.sigmoid().cpu().float().numpy()
            if isinstance(reg_pred, torch.Tensor):
                reg_pred = reg_pred.cpu().float().numpy()

            mask_pred_ = mask_pred[i, 0, :, :]
            mask_pred_resize = imresize(mask_pred_, (w_pad, h_pad))
            mask_pred_resize = imresize(mask_pred_resize[:h_img, :w_img], (w_ori, h_ori))
            ...
            reg_pred1_resize = imresize(reg_pred1_, (w_pad, h_pad))
            reg_pred2_resize = imresize(reg_pred2_, (w_pad, h_pad))
            reg_pred1_resize = imresize(reg_pred1_resize[:h_img, :w_img], (w_ori, h_ori))
            reg_pred2_resize = imresize(reg_pred2_resize[:h_img, :w_img], (w_ori, h_ori))
